---
title: Handle Notes
description: Create, inspect, and modify chart notes.
---
...
| Field | Meaning                                                                                   |
| ----- | ----------------------------------------------------------------------------------------- |
| `t`   | Timing. Pass a raw tick `int`, or a musical tuple such as `(bar, beat, offset)`. |
...
Every typed note accepts `t` as either a raw tick or a tuple:
...
Inside `with m.open_edit()` blocks, tuples resolve with the chart's actual time signatures. Outside an edit transaction, they use the default 4/4 context unless you call the time helpers with explicit beat events. See the [Time & Position](/docs/guides/time) guide for the full timing model.
...
| `gap`   | Segment spacing, as raw ticks or a beat fraction tuple. |
...
# Convert typed notes to raw before low-level inspection.
