...
Margrete stores time as integer ticks. The Python API lets you work at that level when you need exact control, or use musical tuple forms for charting code that is easier to read.

## Ticks

The lowest-level unit of time is the **tick**. Margrete RPC uses a resolution of `1920` ticks per whole note, so common note values map to these tick counts:
...
Positions describe _where_ something starts. Durations describe _how long_ something lasts. Use `div_to_tick(numerator, denominator)` to convert a note value to ticks:
