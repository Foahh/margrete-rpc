...
Transforms are methods on `Note` objects. Use them when you want to adjust existing notes instead of rebuilding them by hand.
...
For `t`, you can also pass a `Position` tuple. This is resolved the same way as [constructor positions](/docs/guides/notes#positioning).
...
`clamp()` keeps note geometry inside a lane range, default to `[0, 16)`.
