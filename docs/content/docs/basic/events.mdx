...
speed. On the Python side they are simple objects in `tx.chart.events`, so most
event edits are just list edits inside an `open_edit()` block.
...
| ---------------------------- | ---------------------------------------- |
| `tx.chart.events.bpm`        | Song tempo changes.                      |
| `tx.chart.events.beat`       | Time-signature changes.                  |
| `tx.chart.events.til`        | Scroll speed for a specific timeline.    |
| `tx.chart.events.note_speed` | Note speed, independent of scroll speed. |
