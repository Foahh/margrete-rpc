...
use crate::chart_mapper::snapshot_for_edit;
use crate::config::ServerConfig;
use crate::deduper::deduplicate;
use crate::logger::Logger;
use crate::meta;
use crate::proto::v1::{envelope, Envelope, ErrorCode, ErrorResponse, StatusResponse};
use crate::session::MargreteSession;
use crate::transaction::apply_edit;
use std::sync::Mutex;
    ...
    fn route_inner(&self, request: &Envelope) -> crate::error::Result<Envelope> {
        ...
        match &request.body {
            Some(envelope::Body::BeginEditRequest(req)) => {
                ...
                let mut begin = crate::proto::v1::BeginEditResponse {
            ...
            Some(envelope::Body::UndoRequest(_)) => {
                ...
                Ok(Envelope {
                    ...
                    body: Some(envelope::Body::UndoResponse(
                        crate::proto::v1::UndoResponse { success },
                    )),
                })
            }
            Some(envelope::Body::RedoRequest(_)) => {
                ...
                Ok(Envelope {
                    ...
                    body: Some(envelope::Body::RedoResponse(
                        crate::proto::v1::RedoResponse { success },
                    )),
                })
            }
            Some(envelope::Body::CurrentTickRequest(_)) => {
                ...
                Ok(Envelope {
                    ...
                    body: Some(envelope::Body::CurrentTickResponse(
                        crate::proto::v1::CurrentTickResponse { current_tick: tick },
                    )),
                })
            }
            _ => {
