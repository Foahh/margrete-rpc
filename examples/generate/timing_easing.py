...
def build_chart() -> tuple[list[ChartNote], list[TimelineSpeedEvent]]:
    ...
    for name in EASING_NAMES:
        ...
        events.extend(
            timing_easing(
                t0=t0, t1=t1, start_speed=0.5, end_speed=2.0, count=16, easing=name, til=TIL
            )
        )
    ...
    for name in EASING_NAMES:
        ...
        events.extend(
            timing_easing(
                t0=t0, t1=t1, start_speed=2.0, end_speed=0.5, count=16, easing=name, til=TIL
            )
        )
    ...
    for name in extreme_easings:
        ...
        events.extend(
            timing_easing(
                t0=t0, t1=t1, start_speed=0.0, end_speed=3.0, count=16, easing=name, til=TIL
            )
        )
    ...
    for start, end in speed_pairs:
        ...
        events.extend(
            timing_easing(
                t0=t0, t1=t1, start_speed=start, end_speed=end, count=16, easing="in_expo", til=TIL
            )
        )
