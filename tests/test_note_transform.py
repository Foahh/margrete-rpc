...
def test_align_accepts_interval_like():
    assert Tap(t=503, x=0, w=2).aligned((1, 4)).t == 480
...
def test_clamp_left_moves_x_and_shrinks_w():
    tap = Tap(t=0, x=2, w=4)
    tap.clamp(left=4)
    assert tap.x == 4
...
def test_clamp_left_minimum_width_one():
    tap = Tap(t=0, x=0, w=1)
    tap.clamp(left=3)
    assert tap.x == 3
    assert tap.w == 1
...
def test_clamp_right_shrinks_w():
...
def test_clamp_right_note_entirely_past_boundary():
...
def test_clamp_no_change_when_within_bounds():
    tap = Tap(t=0, x=4, w=4)
    tap.clamp(left=0, right=STANDARD_FIELD_WIDTH)
    assert tap.x == 4
    assert tap.w == 4
...
def test_clamp_tap_with_air_clamps_air_too():
    from margrete_rpc.chart.notes import Air, AirDirection

    tap = Tap(t=0, x=0, w=6).with_air(Air(AirDirection.UP, t=0, x=0, w=6))
    tap.clamp(left=2, right=14)
...
def test_clamp_rejects_invalid_bounds():
    ...
    with pytest.raises(ValueError):
        Tap(t=0, x=0, w=2).clamp(left=8, right=4)
    with pytest.raises(ValueError):
        Tap(t=0, x=0, w=2).clamp(left=4, right=4)
