...
Giving the agent the `margrete_rpc` source code yields the most accurate API information.
...
<Tabs items={["Installed Package", "Clone the Repo"]}>
  ...
  <Tab value="Clone the Repo">
    ...
    Add the `python/margrete_rpc/` directory to the agent's context.
...
<Callout type="info">
  Provide `https://mg.foahh.com/llms.txt` to the agent; it can start from there and fetch whichever
  documentation pages it needs.
</Callout>
