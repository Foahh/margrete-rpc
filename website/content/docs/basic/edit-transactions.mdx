...
Open a transaction, modify the chart, then exit the `with{:py}` block.
...
The outer `with Margrete() as m` closes the persistent RPC connection when your script is done.

For the exact method signature, see [`Margrete.open_edit()`](/docs/reference/client/Margrete#functions) and the [`EditTransaction`](/docs/reference/transaction/EditTransaction) reference.
...
2. **Parse** - The Python API converts [`RawNote`](/docs/reference/chart/notes/raw/RawNote) objects to [`Note`](/docs/reference/chart/notes/shared/Note) objects such as [`Tap`](/docs/reference/chart/notes/ground/Tap)
   and [`Slide`](/docs/reference/chart/notes/long/Slide). <br/> If `raw_notes=True`, this step keeps every note as `RawNote` instead.
...
The [`EditTransaction`](/docs/reference/transaction/EditTransaction) object has three things you will use most often:

| Object            | What it is for                                                                                   |
| ----------------- | ------------------------------------------------------------------------------------------------ |
| `tx.chart.notes`  | A mutable `list` of chart notes. See the [`Chart`](/docs/reference/chart/chart/Chart) reference. |
...
`tx.chart.events` groups chart events by kind. See [Events](/docs/basic/events) for examples that add, mutate, and filter each event list:
...
Notes are regular objects. Transform methods mutate them in place. See [Note Transforms](/docs/basic/note-transforms) for examples and the [`Note`](/docs/reference/chart/notes/shared/Note#functions) reference for exact signatures.
...
By default, the Python API parses recognized [`RawNote`](/docs/reference/chart/notes/raw/RawNote) objects into [`Note`](/docs/reference/chart/notes/shared/Note) objects such as [`Tap`](/docs/reference/chart/notes/ground/Tap) or [`Slide`](/docs/reference/chart/notes/long/Slide). If it sees an unsupported note, it keeps that note as `RawNote` instead of dropping it.
...
Use [`m.current_tick()`](/docs/reference/client/Margrete#functions) when you need the live cursor position outside a transaction:
