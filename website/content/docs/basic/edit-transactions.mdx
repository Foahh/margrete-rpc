...
`open_edit()` is the normal way to script charting with Python:
...
The edit is sent to Margrete when the `with m.open_edit() as tx:{:py}` block exits normally. If this block raises an
...
1. **Snapshot** - Margrete sends the current chart and other information to Python.
