...
Note classes live in [`margrete_rpc.chart.notes`](/docs/reference/chart/notes). In normal [edit transactions](/docs/basic/edit-transactions), `tx.chart.notes` contains these [`Note`](/docs/reference/chart/notes/shared/Note) objects when Margrete RPC recognizes the note tree, and [`RawNote`](/docs/reference/chart/notes/raw/RawNote) objects for structures that need low-level handling.
...
| Field | Meaning                                                                                                                    |
| ----- | -------------------------------------------------------------------------------------------------------------------------- |
| `t`   | Timing. Pass a raw tick `int`, or a [`Position`](/docs/reference/chart/time/Position) tuple such as `(bar, beat, offset)`. |
...
Every [`Note`](/docs/reference/chart/notes/shared/Note) accepts `t` as either a raw tick or a [`Position`](/docs/reference/chart/time/Position) tuple:
...
Use [`Tap`](/docs/reference/chart/notes/ground/Tap) for a basic ground tap.
...
Use [`Flick`](/docs/reference/chart/notes/ground/Flick) for left/right flicks. `dir` accepts [`FlickDirection`](/docs/reference/chart/notes/direction/FlickDirection) or a string.
...
Use [`Extap`](/docs/reference/chart/notes/ground/Extap) for directional ex-taps. `dir` accepts [`ExtapDirection`](/docs/reference/chart/notes/direction/ExtapDirection) or a string.
...
Use [`Damage`](/docs/reference/chart/notes/ground/Damage) for penalty notes.
...
For generated slide paths instead of hand-written joints, see [Curved Slides](/docs/advanced/curved-slides).

### Hold

A [`Hold`](/docs/reference/chart/notes/long/Hold) starts at the begin point and ends at a single step joint.
...
A [`Slide`](/docs/reference/chart/notes/long/Slide) can move through step and control joints.
...
[`AirCrush`](/docs/reference/chart/notes/long/AirCrush) is an air-lane long note. It uses the same begin/joint structure as a slide, plus:

| Field   | Meaning                                                                                     |
| ------- | ------------------------------------------------------------------------------------------- |
| `h`     | Air height at the begin or joint.                                                           |
| `gap`   | Segment spacing, as raw ticks or a [`Division`](/docs/reference/chart/time/Division) tuple. |
| `color` | Crush color, as [`ColorValue`](/docs/reference/chart/notes/color/ColorValue) or a string.   |
...
You can use [`add_air()`](/docs/reference/chart/notes/air/_AirAttachable#functions) or assign `.air` directly:
...
Use [`with_air()`](/docs/reference/chart/notes/air/_AirAttachable#functions) when you want to keep the original note unchanged:
...
[`AirDirection`](/docs/reference/chart/notes/direction/AirDirection) values are `"up"`, `"down"`, `"up_left"`, `"up_right"`, `"down_left"`, and `"down_right"`.

### Air on ground notes

Attach [`Air`](/docs/reference/chart/notes/air/Air) above a tap, flick, extap, or damage note.
...
Attach [`Air`](/docs/reference/chart/notes/air/Air), [`AirSlide`](/docs/reference/chart/notes/air/AirSlide), or [`AirHold`](/docs/reference/chart/notes/air/AirHold) to a long note's end joint.
...
[`RawNote`](/docs/reference/chart/notes/raw/RawNote) is the [low-level Margrete representation](https://github.com/MargreteSDK/MargretePluginSDK/blob/b8d0c87125e090880a75f6f0ff57a773510299a8/include/MargretePlugin.h#L126-L138). You usually do not need it for normal scripting.
...
Call [`note.validate()`](/docs/reference/chart/notes/shared/Note#functions) when you want to catch invalid notes early.
